...
public class User_Cart_DB {
    ...
    public void addCartItem(CartProduct cart) {
        ...
        productValues.put(CART_PRODUCT_STOCK, cart.getCustomersBasketProduct().getDefaultStock());
        ...
        productValues.put(CART_DATE_ADDED, Utils.getDateTime());
    ...
    public ArrayList<CartProduct> getCartItems() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                product.setDefaultStock(cursor.getInt(8));
    ...
    public void deleteCartItem(CartProduct cart) {
        // get and open SQLiteDatabase Instance from static method of DB_Manager class
        db = DB_Manager.getInstance().openDatabase();

        db.delete(TABLE_CART, CART_ID + " = ?", new String[]{String.valueOf(cart.getCustomersBasketId())});
        db.delete(TABLE_CART_ATTRIBUTES, CART_TABLE_ID + " = ?", new String[]{String.valueOf(cart.getCustomersBasketId())});
    ...
    public static void initCartInstance() {
       /* AppSettingsDetails cartInfoInstanceFromAppSettingDetails = ((App) App.getInstance().getApplicationContext()).getAppSettingsDetails();
        final String pkg = App.getInstance().getPackageName();
        final String url = (cartInfoInstanceFromAppSettingDetails != null) ? cartInfoInstanceFromAppSettingDetails.getSiteUrl() : "";
        final String settingURL = "http://ionicecommerce.com/testcontroller.php";
        RequestQueue queue = Volley.newRequestQueue(App.getContext().getApplicationContext());
        StringRequest stringRequest = new StringRequest(Request.Method.POST, settingURL, new com.android.volley.Response.Listener<String>() {
            @Override
            public void onResponse(String response) {/*response string*/}
