...
public class CouponsAdapter extends RecyclerView.Adapter<CouponsAdapter.MyViewHolder> {
    ...
    public CouponsAdapter(Context context, List<CouponsInfo> couponsList, Boolean isRemovable) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        holder.coupon_discount.setText(Utils.getStringRes(R.string.egp)+ new DecimalFormat("#0.00", DecimalFormatSymbols.getInstance(Locale.ENGLISH)).format(Double.parseDouble(coupon.getDiscount())));
        ...
        if (coupon.getDiscountType().equalsIgnoreCase("fixed_cart") || coupon.getDiscountType().equalsIgnoreCase("fixed_product")) {
            holder.coupon_amount.setText(Utils.getStringRes(R.string.egp)+ coupon.getAmount());
        }
