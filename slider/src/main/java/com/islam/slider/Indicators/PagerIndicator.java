package com.islam.slider.Indicators;
...
public class PagerIndicator extends LinearLayout implements ViewPagerEx.OnPageChangeListener{
    ...
    /**
     * bind this Indicator with {@link com.islam.slider.Tricks.ViewPagerEx}
     */
    ...
    /**
     *This value is from {@link com.islam.slider.SliderAdapter} getRealCount() represent
     *
    ...
    public PagerIndicator(Context context, AttributeSet attrs) {
        ...
        mDefaultSelectedColor = attributes.getColor(R.styleable.PagerIndicator_selected_color, Color.rgb(203, 45, 62));
        mDefaultUnSelectedColor = attributes.getColor(R.styleable.PagerIndicator_unselected_color, Color.argb(255,255,255,255));
        mBorderColor = attributes.getColor(R.styleable.PagerIndicator_border_color, Color.rgb(203, 45, 62));
        ...
        mBorderWidth = attributes.getDimensionPixelSize(R.styleable.PagerIndicator_border_width,(int)pxFromDp(2));
        ...
        setDefaultIndicatorColor(mDefaultSelectedColor, mDefaultUnSelectedColor,mBorderColor);
     ...
     */
    private void setDefaultIndicatorColor(int selectedColor, int unselectedColor,int borderColor){
        if(mUserSetSelectedIndicatorResId == 0){
            mSelectedGradientDrawable.setColor(selectedColor);
            mSelectedGradientDrawable.setStroke((int) pxFromDp(mBorderWidth/2),selectedColor);
        }
        if(mUserSetUnSelectedIndicatorResId == 0){
            mUnSelectedGradientDrawable.setColor(unselectedColor);
            mUnSelectedGradientDrawable.setStroke((int) pxFromDp(mBorderWidth/2),borderColor);
        }
        resetDrawable();
    }
    ...
    private void setDefaultSelectedIndicatorSize(float width, float height, Unit unit){
        if(mUserSetSelectedIndicatorResId == 0){
            ...
            float h = height;
            if(unit == Unit.DP){
                w = pxFromDp(width)+mBorderWidth;
                h = pxFromDp(height)+mBorderWidth;
            }
            else {
                w=w+mBorderWidth;
                h=h+mBorderWidth;
