 ...
 * I provide two example: {@link com.islam.slider.SliderTypes.DefaultSliderView} and
 * {@link com.islam.slider.SliderTypes.TextSliderView}
...
public abstract class BaseSliderView {
     ...
     * @return
     */
    public BaseSliderView image(String url,boolean concatToBaseUrl){
        if(mFile != null || mRes != 0){
            throw new IllegalStateException("Call multi image function," +
                    "you only have permission to call it once");
        }
        if(concatToBaseUrl)
        mUrl = BASE_URL.concat(url);
        else mUrl=url;
        return this;
    }
    ...
    void bindEventAndShow(final View v, ImageView targetImageView){
        ...
        if(mUrl!=null){
            rq = p.load(mUrl).placeholder(R.drawable.logo);
        }else if(mFile != null){
            rq = p.load(mFile);
        }else if(mRes != 0){
            rq = p.load(mRes).placeholder(R.drawable.logo);
        }else{
            return;
        }
        ...
        switch (mScaleType){
            case Fit:
                ...
                break;
            case CenterCrop:
                rq=rq.fit().centerCrop();
                break;
            case CenterInside:
                rq=rq.fit().centerInside();
                break;
        }
